...
export function createChatScreen({ role = globalThis.window?.currentRole || 'mistress', onBack } = {}) {
  ...
  backBtn.onclick = () => {
    ...
    globalThis.window?.dispatchEvent(new CustomEvent('mistress-x:navigate', {
      detail: { route: 'role-picker' },
    }));
  };
  ...
  const chatList = createChatList(chatWindow, role);
