...
export function createClosedLotsPanel() {
  const closedLots = getClosedLots();
  ...
  panel.innerHTML = `
    ...
    <p class="mx-auction-description">Closed auctions will appear here for follow-up review.</p>
  ...
  closedLots.slice(0, 12).forEach((lot) => {
    ...
    const followupLabel = getFollowupLabel(lot);
    card.innerHTML = `
      ...
      <div class="mx-auction-pill-row">
        <span class="mx-auction-pill is-active">${followupLabel}</span>
